    ...
    <div className="p-4 pb-20 lg:p-6 lg:pb-6">
      ...
      <div className="mb-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <select
            value={etat || ''}
            onChange={(e) => { setEtat(e.target.value || undefined); setPage(1); }}
            className="rounded-md border border-input px-3 py-1.5 text-sm"
          >
          ...
          <select
            value={typeItem || ''}
            onChange={(e) => { setTypeItem(e.target.value || undefined); setPage(1); }}
            className="rounded-md border border-input px-3 py-1.5 text-sm"
          >
          ...
          <select
            ...
            onChange={(e) => { setLocalisationId(e.target.value ? Number(e.target.value) : undefined); setPage(1); }}
            className="rounded-md border border-input px-3 py-1.5 text-sm"
          >
      ...
      {data && (
        <p className="mb-3 text-sm text-muted-foreground">
          {data.total} item{data.total > 1 ? 's' : ''} trouve{data.total > 1 ? 's' : ''}
        </p>
      )}
                ...
                {data?.items.map((item: any) => (
                  <tr key={item.id} className="border-t border-border hover:bg-muted/50">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {item.commentaireEtat && <div className="mt-0.5 text-xs text-muted-foreground">{item.commentaireEtat}</div>}
                    </td>
          ...
          <div className="space-y-2 md:hidden">
            {data?.items.map((item: any) => (
              <div key={item.id} className="rounded-lg border border-border p-3">
                ...
                {item.commentaireEtat && <p className="mt-1 text-xs text-muted-foreground">{item.commentaireEtat}</p>}
              </div>
            ))}
          </div>
